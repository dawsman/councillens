...
- [The Statements of Accounts](https://www.norwich.gov.uk/your-council-explained/transparency-and-accountability/statement-accounts) for 2021-22 to 2025-26, plus the audit certificates for 2023-24 and 2024-25. The narrative report at the front of each one carries the variance against budget and the capital outturn, and those are the numbers every money measure uses.
...
Two decisions worth recording. The provisional outturn reports to Cabinet each June are small and would have been the obvious thing to bank, but a provisional figure and an audited-basis one are different things, and every money measure here is scored against the accounts. That is a choice about which record to use. It is not a claim that the two disagree: the provisional reports are not in this archive, nobody working on this topic has read one, and nothing here quotes them. And the accounts before 2021-22 are published by the council but are not banked here, to keep the repository a sensible size; that is a gap rather than a silence.
...
Norwich publishes its own traffic-light framework: a target and an intervention level for every corporate indicator, reported to Cabinet each quarter. Where it does, the `council-threshold-v1` rule uses both, so the colour here is the colour on the council's own dashboard rather than one of our devising. Where a council publishes a target but no intervention level, `target-met-v1` applies a 10% band, and that band is ours.

On the money measures the council publishes no target at all, and it is worth saying plainly what that means. Nothing in Norwich's papers says how close to its own budget is close enough, how much of a building programme ought to be delivered, or what share of a savings plan counts as delivered. So on those we compare the council against its own plan and we choose where the line falls: 2% and 5% off budget, 90% and 70% of a capital programme, 95% and 80% of a savings target, a tenth below a reserves floor. Five of the seven rules carry a line of ours in that way; two — `council-threshold-v1` and `promise-kept-v1` — carry none. Every one of those lines is printed beside the rule that uses it, with the reason we drew it there, so a reader who thinks it is in the wrong place can say so. The numbers on both sides of every comparison are the council's.

Grey is a first-class answer. Nine of the forty-three measures currently on the site are grey, and most of them are about the same thing: Norwich publishes the savings it has identified when it sets a budget, and never publishes how much of that was actually delivered. That is the most obvious accountability question about a council's money and it cannot be answered from Norwich's published papers.
